...
public:
	...
	vec2f vers_point_plus_proche(vec2f p) { 
        ...
        vec2f d1 = p0 + v_dir*(d_proj - door_size/2) - p;
        vec2f d2 = p0 + v_dir*(d_proj + door_size/2) - p;
        vec2f dx = vec2f( p0.x - p.x, 0);
        
        float a = std::abs(p0.x - p.x);
        if (std::abs(p.y-d*door_pos) < door_size/2)
        {
            if (d1.squareDistance() < d2.squareDistance()) {
                return d1;
            } else {
                return d2;
            }
        ...
        if ( ( (d_proj - door_size/2) > door_pos*d ) && ( (d_proj - door_size/2) > door_pos*d ) ) { //le projeté est "dans la porte"
            if ( d_proj < door_pos*d ) { // plus près de p0
                return ( p0 + v_dir*(d_proj - door_size/2) - p);
